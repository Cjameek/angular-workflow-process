...
import { RuleProperty } from '../../../approval-workflow/data-access/models/rule-model';

@Component({
  selector: 'dropdown-rule-property',
  template: `
    <label [attr.id]="labelId()" [attr.for]="name()">Rule Property</label>
    ...
    <small [attr.id]="descriptionId()">Select a property from order type to use as basis for approval rule.</small>
  `,
  imports: [FormsModule]
})
